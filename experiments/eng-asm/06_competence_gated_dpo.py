...
def parse_args():
    ...
    p.add_argument("--competence", type=str, default="max",
                   choices=["max", "mean"],
                   help="Base competence per example: max or mean candidate reward.")
...
class CGPODataset(Dataset):
    ...
    def __init__(self, scored_csv, tokenizer, ip, cfg, logger,
        ...
        for _, row in df.iterrows():
            ...
            if gap < min_gap:
                skipped += 1; continue
            comp = max(r0, r1) if competence == "max" else 0.5 * (r0 + r1)
            rows.append({
                "src": row["src"],
...
def main():
    ...
    tag = args.tag or f"cgpo-g{args.gamma}"
    run_name = f"{exp}-{tag}"
