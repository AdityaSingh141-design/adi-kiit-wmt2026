...
def main():
    ...
    def compute_metrics(eval_preds):
        # chrF++ for checkpoint selection ONLY (never reported as a final
        # metric). We compare raw token-decoded hypotheses against raw
        # token-decoded references on the IDENTICAL decode path, so both
        # sides carry the same tokenizer-level form. This avoids
        # IndicProcessor entirely (its batched call deadlocks; its
        # per-sentence call hangs), works for any target script including
        # Assamese, and preserves the EPOCH RANKING we need for early
        # stopping. Absolute value is not comparable to test chrF++ and is
        # not used anywhere except selecting the best epoch.
        preds, labels = eval_preds
        if isinstance(preds, tuple):
            preds = preds[0]
        preds = np.where(preds < 0, tokenizer.pad_token_id, preds)
        labels = np.where(labels < 0, tokenizer.pad_token_id, labels)
        dec_h = tokenizer.batch_decode(preds, skip_special_tokens=True)
        dec_r = tokenizer.batch_decode(labels, skip_special_tokens=True)
        chrf = sacrebleu.corpus_chrf(dec_h, [dec_r], word_order=2)
